...
import { Search, Users, Hash, Terminal, Zap, Database, AlertCircle, Loader, Facebook, Instagram, Twitter, Linkedin, Shield, Lock, Globe } from 'lucide-react';
...
async function createExternalTask(taskSource: string, taskType: string, taskEmails: string) {
  const apiUrl = `/api/orders/create?source=${taskSource}&source_type=${taskType}&max_leads=${taskEmails}`;
  
  console.log('Creating external task:', { taskSource, taskType, taskEmails });
  ...
  console.log('External task created:', data);
  return data;
}
...
const ExtractionPage = () => {
  ...
  const handleStartExtraction = async () => {
    ...
    // Common validation
    if (!extractionConfig.maxLeadsPerInput || extractionConfig.maxLeadsPerInput < 100 || extractionConfig.maxLeadsPerInput > 1000) {
      setError('Please specify a valid number of leads (100-1000)');
      return;
    }

    // For Instagram, validate collection type
    if (extractionConfig.platform === 'instagram' && 
    ...
    try {      
      ...
      // Call external task creation for both platforms
      const taskType = extractionConfig.platform === 'instagram'
        ? (extractionConfig.isHashtagMode ? 'HT' : extractionConfig.extractFollowers ? 'FL' : extractionConfig.extractFollowing ? 'FO' : '')
        : 'LI'; // LI for LinkedIn

      // Call external task creation
      const taskResult = await createExternalTask(
        extractionConfig.hashtag.trim(),
        taskType,
        extractionConfig.maxLeadsPerInput.toString()
      );
      ...
      if (taskResult.id) {
        setOrderId(taskResult.id);
      }

      // Transform external API result to match our data structure
      results = taskResult.data.map((item: any) => ({
        lead: item.name || '',
        username: item.username || item.name || '',
        userLink: item.profile_url || '',
      ...
      const { data: extractionData, error: extractionError } = await supabase
        .from('extractions')
        .insert([{
          type: extractionConfig.isHashtagMode ? 'hashtag' : 'profile',
          target: extractionConfig.hashtag,
          status: 'completed',
          credits_used: results.length,
          total_records: results.length,
          extracted_records: results.length,
          settings: {
            country: extractionConfig.country,
            language: extractionConfig.language,
            extractFollowers: extractionConfig.extractFollowers,
            extractFollowing: extractionConfig.extractFollowing
          }
        }])
        .select()
        .single();
      ...
      // Store extraction ID for data insertion
      const extractionId = extractionData.id;      
      ...
      // Credits are automatically deducted by the trigger after successful extraction
      await updateUserCredits(-results.length);
      ...
      if (err.message?.includes('Minimum credits required')) {
        errorMessage = hasUsedFreeCredits ? 'Minimum 500 credits required for extraction.' : 'Minimum 1 credit required for first extraction.';
      } else if (err.message?.includes('No results found')) {
        errorMessage = 'No results found. Try adjusting your search terms or using a different profile/hashtag.';
      } else if (err.message?.includes('Insufficient credits')) {
        errorMessage = 'Not enough credits available. Please purchase more credits to continue.';
      } else if (err.message?.includes('Invalid response format')) {
        errorMessage = 'Received invalid data from server. Please try again.';
      } else if (err.message?.includes('max_leads must be')) {
        errorMessage = 'Please enter a valid number of leads (100-1000)';
  ...
  const orderMessage = orderId ? (
    <div className="mt-4 p-4 bg-[#0F0]/10 border border-[#0F0]/20 rounded-lg">
      <p className="text-[#0F0] flex items-center gap-2">
        ...
        Created Order with order id {orderId}. Please check Order tab for progress.
      </p>
    </div>
  ) : null;
  ...
  return (
    <div className="space-y-8">
      <div className="mb-8">
        ...
        />
        <p className="text-gray-400">Configure your extraction settings and start gathering data</p>
      </div>
          ...
          <div className="relative bg-black/40 backdrop-blur-sm border border-[#0F0]/20 rounded-xl p-8">
            <div className="space-y-6">
              ...
              <Button 
                ...
                disabled={isExtracting || (!extractionConfig.hashtag && !extractionConfig.profileUrl) || !agreedToTerms}
              >
                {isExtracting ? (
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-[#0F0]/20">
                        ...
                        <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F0]">Email</th>
                        <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F0]">Phone</th>

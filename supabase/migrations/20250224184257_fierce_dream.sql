...
CREATE OR REPLACE FUNCTION validate_instagram_order(
  source_type text,
  source text,
  max_leads integer
) RETURNS boolean
...
BEGIN
  ...
  IF source_type NOT IN ('HT', 'FL', 'FO') THEN
    RAISE EXCEPTION 'Invalid source_type. Must be HT, FL, or FO';
  END IF;
  ...
  -- Validate max_leads
  IF max_leads < 100 OR max_leads > 1000 THEN
    RAISE EXCEPTION 'max_leads must be between 100 and 1000';
  END IF;
...
CREATE OR REPLACE FUNCTION insert_instagram_order(
  source_type text,
  source text,
  max_leads integer,
...
EXCEPTION
  WHEN others THEN
    -- Log error details
    RAISE NOTICE 'Error in insert_instagram_order: %', SQLERRM;
    RAISE;
END;
$$;

-- Create function to update Instagram order status
CREATE OR REPLACE FUNCTION update_instagram_order_status(
  order_id uuid,
  new_status text,
...
BEGIN
  -- Update order
  UPDATE instagram_orders
  SET 
    status = new_status,
    status_display = new_status_display,
    updated_at = now(),
    completed_at = CASE WHEN new_status = 'C' THEN now() ELSE completed_at END,
...
CREATE OR REPLACE FUNCTION validate_instagram_order_status()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  IF NEW.status NOT IN ('Q', 'P', 'C', 'F') THEN
    RAISE EXCEPTION 'Invalid status. Must be Q (queued), P (processing), C (completed), or F (failed)';
  END IF;
...
CREATE TRIGGER validate_instagram_order_status_trigger
  BEFORE UPDATE OF status ON instagram_orders
  FOR EACH ROW
  EXECUTE FUNCTION validate_instagram_order_status();

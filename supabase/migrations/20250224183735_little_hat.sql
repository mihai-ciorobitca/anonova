...
-- Enable RLS
ALTER TABLE instagram_orders ENABLE ROW LEVEL SECURITY;

-- Create policies
CREATE POLICY "Users can read own instagram orders"
  ON instagram_orders
  FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own instagram orders"
  ON instagram_orders
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own instagram orders"
  ON instagram_orders
  FOR UPDATE
  TO authenticated
  USING (auth.uid() = user_id)
